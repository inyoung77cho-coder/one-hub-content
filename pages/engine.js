...
const STYLES = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .engine-orb.on { background:var(--green-dim); animation:orb-pulse 3s ease-in-out infinite; }
  .engine-orb.off { background:var(--red-dim); }
  @keyframes orb-pulse { 0%,100%{box-shadow:0 0 0 6px rgba(0,208,132,.08)} 50%{box-shadow:0 0 0 10px rgba(0,208,132,.04)} }
  ...
  .total-label { font-family:var(--mono); font-size:10px; color:var(--text3); text-transform:uppercase; }
  ...
  .tl-item { display:flex; align-items:center; gap:14px; padding:7px 0; }
  .tl-time { font-family:var(--mono); font-size:11px; font-weight:700; width:40px; text-align:right; flex-shrink:0; color:var(--text3); }
  .tl-dot { width:10px; height:10px; border-radius:50%; flex-shrink:0; z-index:1; border:2px solid var(--bg); }
  .tl-dot.done { background:var(--text3); }
  .tl-dot.soon { background:var(--yellow); animation:pulse-yellow 1.5s infinite; }
  ...
  .strategy-item { background:var(--bg3); border:1px solid var(--border); border-radius:8px; padding:14px 16px; display:grid; grid-template-columns:36px 1fr 60px; gap:12px; align-items:start; }
  ...
  .strategy-pct { font-family:var(--mono); font-size:18px; font-weight:700; color:var(--blue); text-align:right; }
  .strategy-pct-label { font-family:var(--mono); font-size:9px; color:var(--text3); text-align:right; }
  .weight-bar-bg { height:3px; background:var(--border); border-radius:2px; margin-top:8px; grid-column:2/4; overflow:hidden; }
  .weight-bar-fill { height:100%; background:var(--blue); border-radius:2px; }
  ...
  .footer-ts { text-align:center; font-family:var(--mono); font-size:10px; color:var(--text3); margin-top:32px; }
          ...
          {(strategy || []).map((s) => (
            <div key={s.id} className="strategy-item">
              ...
              <div className="weight-bar-bg">
                <div className="weight-bar-fill" style={{ width: `${s.weight}%` }} />
              </div>
            </div>
          ))}
        </div>
...
export default function EnginePage() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCountdown((c) => {
        if (c <= 1) { fetchData(); return POLL_SEC; }
        return c - 1;
      });
    }, 1000);
  ...
  return (
    <>
      ...
      <div className="page">
        {isOffline && (
          <div className="offline-banner">AWS 서버 응답 없음</div>
        )}
        <div className="refresh-bar">
          <div className={`pulse-dot ${isOffline ? "offline" : ""}`} />
          <span>{loading ? "연결 중..." : isOffline ? "서버 오프라인" : `${countdown}초 후 갱신`}</span>
        </div>
        {loading ? (
