...
export default function StrategiesPage() {
  const statusColor = (s) => s === "ACTIVE" ? "#00AA55" : "#CC8800";
  const statusBg = (s) => s === "ACTIVE" ? "#E8F8EF" : "#FFF8E6";
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#0a0c10", color: "#1A1A1A", fontFamily: "'Noto Sans KR', sans-serif", padding: "0 0 80px" }}>
        <main style={{ maxWidth: "900px", margin: "0 auto", padding: "40px 24px" }}>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E0DDD4", borderRadius: "12px", padding: "20px 24px", marginBottom: "24px" }}>
            ...
            <div style={{ display: "flex", gap: "20px", marginTop: "10px" }}>
              {strategies.filter(s => s.weight > 0).map(s => (
                <span key={s.id} style={{ fontFamily: "monospace", fontSize: "10px", color: "#6A6660" }}>{s.icon} {s.weight}%</span>
              ))}
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {strategies.map(s => (
              <div key={s.id} style={{ background: "#FFFFFF", border: "1px solid #E0DDD4", borderRadius: "12px", padding: "22px 24px", opacity: s.status === "ACTIVE" ? 1 : 0.6 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <span style={{ fontSize: "22px" }}>{s.icon}</span>
                    <div>
                      <div style={{ fontFamily: "monospace", fontSize: "13px", fontWeight: 700, color: "#1A1A1A" }}>{s.name}</div>
                      <div style={{ fontFamily: "monospace", fontSize: "10px", color: "#9A9690", marginTop: "2px" }}>WEIGHT {s.weight}%</div>
                    </div>
                  </div>
                ...
                <p style={{ fontSize: "12px", color: "#6A6660", lineHeight: 1.6, margin: "0 0 14px" }}>{s.desc}</p>
