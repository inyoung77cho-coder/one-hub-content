...
const strategies = [
  {
    ...
    status: "ACTIVE",
    weight: 40,
    desc: "머신러닝 모델이 종목별 매수/매도 확률을 스코어링. 과거 패턴 학습 기반.",
    rules: ["확률 > 0.65 → 매수 후보", "확률 < 0.35 → 매도 후보", "0.35~0.65 → 관망"],
  },
  {
    id: "final_score",
    name: "Final Score",
    ...
    status: "ACTIVE",
    weight: 35,
    desc: "RSI, MACD, 볼린저밴드 통합 기술적 점수. 0~100 스케일.",
    rules: ["Score ≥ 70 → 강매수", "Score 50~69 → 약매수", "Score < 50 → 차단"],
  },
  {
    id: "risk_mgmt",
    name: "리스크 관리",
    ...
    status: "ACTIVE",
    weight: 25,
    desc: "손절/익절 자동 실행 및 종목별 비중 제한으로 최대 손실 통제.",
    rules: ["손절 기준: -5%", "1종목 최대 비중: 30%", "장 외 시간 API 완전 차단"],
  },
  {
    id: "macro_filter",
    name: "매크로 필터",
    ...
    status: "BUILDING",
    weight: 0,
    desc: "VIX, Fear&Greed, 금리 데이터로 시장 국면 판단 후 전체 매매 강도 조절.",
    rules: ["VIX > 30 → 전체 차단", "Fear < 20 → 비중 50% 축소", "금리 급등 → 성장주 차단"],
  },
];
...
export default function StrategiesPage() {
  ...
  return (
    <>
      <Head><title>Strategies — ONE-HUB</title></Head>
        ...
        <header style={{ borderBottom: "1px solid #1e2530", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0f1218", position: "sticky", top: 0, zIndex: 100 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
            ...
            <nav style={{ display: "flex", gap: "20px" }}>
              {[["DAILY","/daily"],["WEEKLY","/weekly"],["ENGINES","/engines"],["STRATEGIES","/strategies"],["ABOUT","/about"]].map(([label, href]) => (
                <Link key={href} href={href} style={{ fontFamily: "monospace", fontSize: "11px", color: href === "/strategies" ? "#00d084" : "#4a5568", textDecoration: "none", letterSpacing: "0.1em" }}>{label}</Link>
              ))}
            </nav>
          </div>
        </header>
            ...
            {strategies.map(s => (
              <div key={s.id} style={{ background: "#0f1218", border: "1px solid #1e2530", borderRadius: "12px", padding: "22px 24px", opacity: s.status === "ACTIVE" ? 1 : 0.6 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                  ...
                  <span style={{ fontFamily: "monospace", fontSize: "10px", fontWeight: 700, padding: "3px 8px", borderRadius: "4px", background: statusBg(s.status), color: statusColor(s.status) }}>
                    {s.status}
                  </span>
                </div>
