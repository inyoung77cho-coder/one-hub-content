...
export default function EnginesPage() {
  ...
  const engines = [
    {
      ...
      desc: "KIS API 연동 자동매매 — ML 시그널 + 기술적 지표 기반 매수/매도 실행",
      status: isOn ? "RUNNING" : "STOPPED",
      version: engineData?.version || "v8.0",
      live: true,
      icon: "⚡",
      link: "/engine",
    },
    {
      id: "macro",
      name: "Macro Engine",
      desc: "글로벌 매크로 지표(Fear&Greed, VIX, 금리) 수집 및 시장 국면 판단",
      status: "BUILDING",
      version: "v0.1",
      live: false,
      icon: "🌐",
    ...
    },
    {
      id: "news",
      name: "News Engine",
      desc: "RSS 피드 + AI 분류로 테마별 뉴스 수집 및 포트폴리오 영향도 분석",
      status: "BUILDING",
      version: "v0.1",
      live: false,
      icon: "📰",
    ...
    },
    {
      id: "report",
      name: "Report Engine",
      desc: "매일 15:30 KST — Claude AI Insight 생성 → GitHub push → Vercel 자동 배포",
      status: "RUNNING",
      version: "v1.0",
      live: true,
      icon: "📊",
  ...
  const statusColor = (s) => {
    if (s === "RUNNING") return "#00d084";
    if (s === "STOPPED") return "#ff4757";
    return "#ffd700";
  };

  const statusBg = (s) => {
    if (s === "RUNNING") return "#003d26";
    if (s === "STOPPED") return "#3d0010";
    return "#3d3200";
  };
      ...
      <div style={{ minHeight: "100vh", background: "#0a0c10", color: "#e8edf5", fontFamily: "'Noto Sans KR', sans-serif", padding: "0 0 80px" }}>
        ...
        <header style={{ borderBottom: "1px solid #1e2530", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0f1218", position: "sticky", top: 0, zIndex: 100 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
            ...
            <nav style={{ display: "flex", gap: "20px" }}>
              {[["DAILY","/daily"],["WEEKLY","/weekly"],["ENGINES","/engines"],["ABOUT","/about"]].map(([label, href]) => (
                <Link key={href} href={href} style={{ fontFamily: "monospace", fontSize: "11px", color: href === "/engines" ? "#00d084" : "#4a5568", textDecoration: "none", letterSpacing: "0.1em" }}>{label}</Link>
              ))}
            </nav>
          </div>
        </header>
        ...
        <main style={{ maxWidth: "900px", margin: "0 auto", padding: "40px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            {engines.map(eng => (
              <div key={eng.id} style={{ background: "#0f1218", border: `1px solid ${eng.live ? "#1e2530" : "#151a22"}`, borderRadius: "12px", padding: "24px", opacity: eng.live ? 1 : 0.6 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "14px" }}>
                  ...
                  <span style={{ fontFamily: "monospace", fontSize: "10px", fontWeight: 700, padding: "3px 8px", borderRadius: "4px", background: statusBg(eng.status), color: statusColor(eng.status) }}>
                    {eng.status}
                  </span>
                </div>
                <p style={{ fontSize: "12px", color: "#8a9ab5", lineHeight: 1.6, margin: "0 0 16px" }}>{eng.desc}</p>
                {eng.link && (
                  <Link href={eng.link} style={{ fontFamily: "monospace", fontSize: "11px", color: "#00d084", textDecoration: "none", letterSpacing: "0.05em" }}>
                    자세히 보기 →
                  </Link>
                )}
                {!eng.link && (
          ...
          {engineData && (
            <div style={{ marginTop: "32px", background: "#0f1218", border: "1px solid #1e2530", borderRadius: "12px", padding: "24px" }}>
              <div style={{ fontFamily: "monospace", fontSize: "11px", letterSpacing: "0.15em", color: "#4a5568", textTransform: "uppercase", marginBottom: "16px" }}>
                Auto Trade — 실시간 상세
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }}>
                {[
                  ["프로세스 수", `${engineData.engine?.process_count ?? "—"}개`],
                  ["PID", engineData.engine?.pids?.join(", ") || "—"],
                  ["보유 종목", `${engineData.holdings?.length ?? 0}종목`],
                ].map(([label, value]) => (
