...
export function getVerdictScorecard(trader = "A", opts = {}) {
  ...
  const avoided = passes.filter((d) => d.ret < 0);          // [S30-5] 관망했는데 내린 것 = 피한 손실(missed 의 대칭)
  ...
  return {
    ...
    // [S30-5] 피한 손실 — 관망 채점의 반쪽(지금까진 놓친 수익만 셌다). 이야기를 다 말하기 위함.
    avoidedCount: avoided.length, avoidedAvg: avoided.length ? avg(avoided.map((d) => d.ret)) : null,
    passRate, tendency,
  };
}
