...
//   ★한계(C-4): 값이 '키 통째(예: 보유목록 배열 전체 문자열)'라 같은 키를 두 기기가 동시 편집하면
//     통값 LWW 로 한쪽 레코드가 통째로 소실되고, 삭제 표식(tombstone)이 없어 삭제가 되살아난다.
//     레코드 단위 병합은 단계적 설계(docs/API_CONTRACT.md '기기 동기화 한계·설계') 참조.
//   (export: 순수함수라 손실 재현 테스트에 사용 — scripts/syncMerge.loss.test.mjs)
