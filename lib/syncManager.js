...
import { RECORD_SYNC_KEYS, TOMB_KEY, mergeTombstones, mergeRecordArray } from "./recordSync";
...
const SYNC_KEYS = [
  ...
  "onehub_re_budget",
  "onehub_re_jeonse",
  "onehub_re_properties",   // [S37-5] 투자용 부동산(보증금 차감) — ledger.js:220 이 읽어 총자산에 반영. 미동기화 시 기기 바꾸면 보증금 차감이 사라져 총자산이 달라졌다.
  ...
  "onehub_life_stage",      // [S31-4] 생애 단계(축적/전환/인출) — 오늘 화면·인출 계획이 읽음
  "onehub_withdraw",        // [S31-4/5] 인출 입력(월 생활비·연금·배당) — 기기 간 동기화
  ...
  TOMB_KEY,                // [C-4 1단계] 삭제 로그(레코드 단위 병합의 tombstone) — 기기 간 삭제 반영에 필요
];
...
//   [C-4 1단계] 배열형 고위험 키(RECORD_SYNC_KEYS)는 '통값 LWW' 가 아니라 '레코드(id) 단위'로 합친다:
//     · 두 기기가 서로 다른 레코드를 각각 추가 → 둘 다 보존(union, 손실1 해소)
//     · 삭제는 tombstone 로그(TOMB_KEY)로 반영 → 지운 레코드가 되살아나지 않음(손실2 해소)
//     스칼라/설정 키는 종전대로 통값 LWW(회귀 없음).
//   (export: 순수함수라 병합 테스트에 사용 — scripts/recordSync.merge.test.mjs / syncMerge.loss.test.mjs)
export function mergePayloads(local, remote, remoteWinsConflicts) {
  local = local || {}; remote = remote || {};
  // 삭제 로그를 먼저 합친다(레코드 병합이 이 결과를 참조). TTL 지난 tombstone 은 여기서 청소.
  const mergedTombs = mergeTombstones(local[TOMB_KEY], remote[TOMB_KEY]);
  const mergedTombsStr = JSON.stringify(mergedTombs);
  const merged = {};
  ...
  keys.forEach((k) => {
    ...
    let val;
    if (k === TOMB_KEY) {
      val = mergedTombsStr;       // 합친 삭제 로그(양 기기 union·TTL 청소)
    } else if (RECORD_SYNC_KEYS[k]) {
      // 레코드 단위 병합. 결과는 정규 JSON 문자열(파싱→병합→직렬화)이라 무변경 시 원본과 동일.
      const arr = mergeRecordArray(inL ? local[k] : [], inR ? remote[k] : [], mergedTombs[k] || {}, remoteWinsConflicts);
