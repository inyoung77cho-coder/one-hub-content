...
export function getTodayCadence({ trader = "A", date, opClass } = {}) {
  ...
  // 월요일 — 지난주 내 판단 요약(판단이 0건이면 반환 안 함)
  if (d.getDay() === 1) {
    let sc = null;
    try { sc = getVerdictScorecard(trader); } catch (e) {}
    if (sc && sc.total > 0) {
      ...
      hooks.push({ key: "weekly", icon: "🗓️", title: "지난주 내 판단", text: `판단 ${sc.total}건${winTxt}${aiTxt}`, href: "/pwa/record" });
    }
  }
