...
export function getTodayCadence({ trader = "A", date, opClass, engine, withdraw } = {}) {
  const d = date instanceof Date ? date : new Date();
  const hooks = [];

  // [S28-8] 엔진 개선 훅 — today.js 가 넘긴 engine 상태로(비동기라 파라미터로 받는다). 조건 맞는 날에만.
  //   승인 대기 제안: 운영자에게만. 개선됨(월요일): 모두에게. 없으면 아무 것도 안 뜬다.
  if (engine) {
    if (engine.isOperator && engine.pendingCount > 0) {
      hooks.push({ key: "engine_pending", icon: "🔧", title: `엔진 개선 제안 ${engine.pendingCount}건`, text: "승인을 기다립니다 · 정비소에서 확인", href: "/pwa?tab=report&sec=maint" });
    }
    if (engine.improvement && engine.improvement.improved && d.getDay() === 1) {
      const im = engine.improvement;
      hooks.push({ key: "engine_improved", icon: "📈", title: "AI가 개선됐습니다", text: `매수 판단 정확도 ${im.from_pct}% → ${im.to_pct}%`, href: "/pwa?tab=report&sec=vs" });
    }
  }
  ...
  // [S31-5] 매월 첫 영업일 — 인출기 커버율(월 단위 주기. 매일 알림 안 함).
  if (isFirstBizDayOfMonth(d) && withdraw && withdraw.stage === "withdraw" && withdraw.coverRate != null) {
    hooks.push({
      key: "withdraw_cover", icon: "🧮", title: "이번 달 생활비 커버율",
      text: `연금·배당이 생활비의 ${withdraw.coverRate}%${withdraw.years != null ? ` · 지속 약 ${withdraw.years}년(0% 가정)` : ""}`,
      href: "/pwa",
    });
  }
