...
import { earn as earnToken } from "./activityToken"; // [S24-12] 활동 토큰(판단 1건)
import { markFunnel } from "./funnel"; // [S30-8] 첫 판단 관문(가장 중요한 이정표)
...
export async function recordDecisionWithPrice({ code, name, decision, trader = "A", source = "manual", priceHint = null, score = null }) {
  ...
  try { earnToken("verdict", trader); } catch (e) {} // [S24-12] 활동 토큰(하루 상한)
  try { markFunnel("first_verdict", trader); } catch (e) {} // [S30-8] 첫 판단 관문
