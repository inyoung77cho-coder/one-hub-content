...
//   - 매도: 수량 차감(0이면 제거)
import { markDeleted } from "./recordSync"; // [C-4] 삭제 시 tombstone(기기 간 삭제 반영)
...
export function buyEtf({ ticker, market, shares, avgPrice, avgCcy = "USD", account = "일반", broker = "", buyDate = "", trader = "A" }) {
  ...
  if (idx >= 0) {
    ...
    cur.updatedAt = Date.now(); // [C-4] 레코드 갱신 시각(기기 간 LWW 기준)
  } else {
    list.push({ id: `${tk}-${avgCcy}-${acct}-${brk}-${trader}`, ticker: tk, market: mkt, shares: qty, avgPrice: px, avgCcy, account: acct, broker: brk, buyDate: buyDate || "", ts: Date.now(), updatedAt: Date.now(), trader });
  }
  write(list);
  return { ok: true };
}
...
export function sellEtf({ ticker, shares, account, trader = "A" }) {
  ...
  for (const h of holds) {
    ...
    h.updatedAt = Date.now(); // [C-4] 수량 변경 반영
    remain -= cut;
  }
  const removedIds = list.filter((h) => !(h.shares > 0)).map((h) => h.id); // [C-4] 0주가 된 레코드
  list = list.filter((h) => h.shares > 0);
  write(list);
  removedIds.forEach((id) => id != null && markDeleted(KEY, id)); // [C-4] 매도 완료분 삭제 로그
...
export function removeEtf({ ticker, account, trader = "A" }) {
  const tk = String(ticker || "").trim().toUpperCase();
  const before = read();
  const match = (h) => h.ticker === tk && (h.trader || "A") === trader && (!account || (h.account || "일반") === account);
  const removedIds = before.filter(match).map((h) => h.id); // [C-4] 삭제 대상 id
  write(before.filter((h) => !match(h)));
  removedIds.forEach((id) => id != null && markDeleted(KEY, id));
  return { ok: true };
}

// [S22-1] 평단 온전성(ETF) — 주식의 saneAvg(고정 300만 상한)과 달리 ETF는 종목가 편차가 커
//   '현재가 대비 배수'로 본다(ManualHoldingsCard 규칙과 동일: 현재가의 10배 초과 또는 1/10 미만).
//   액면분할 전 가격이나 총매입액을 평단 칸에 넣은 오염을 잡는다. 앱은 고치지 않고 묻는다(N6).
//   avgPrice 는 avgCcy(USD/KRW) 단위 → 현재가와 같은 통화(원)로 맞춰 비교.
export function etfAvgKrw(h, fxRate = null) {
  const a = Number(h?.avgPrice);
  if (!(a > 0) || !Number.isFinite(a)) return null;
  return h?.avgCcy === "USD" ? (fxRate ? a * fxRate : null) : a;
}
// curKrw = 1주당 현재가(원). 시세를 모르면(null) 판정 보류=정상 취급(임의 제외 금지).
export function saneEtfAvg(h, curKrw, fxRate = null) {
  if (h?.verified === true) return true;
  const a = Number(h?.avgPrice);
  if (!(a > 0) || !Number.isFinite(a)) return false; // 0·음수·NaN 은 명백한 오류
  const ak = etfAvgKrw(h, fxRate);
  const c = Number(curKrw);
  if (!(ak > 0) || !(c > 0)) return true;             // 환율·시세 없으면 판정 보류
  const r = ak / c;
  return r <= 10 && r >= 0.1;
}

// [S22-1] "이 값이 맞습니다" — 사용자가 확인. 이후 파생계산에 포함하고 다시 묻지 않는다.
export function verifyEtfAvg({ id, trader = "A" }) {
  const list = read();
...
// [S22-1] "평단 수정" — 사용자가 준 값이 곧 평단(가중평균 재계산 아님). 통화는 기존 avgCcy 유지.
export function updateEtfAvg({ id, avgPrice, trader = "A" }) {
  const px = Number(avgPrice);
  if (!(px > 0)) return { ok: false, error: "평단을 정확히 입력하세요." };
  const list = read();
  const h = list.find((x) => x.id === id && (x.trader || "A") === trader);
  if (!h) return { ok: false, error: "종목을 찾을 수 없습니다." };
  h.avgPrice = (h.avgCcy === "USD") ? Math.round(px * 1e4) / 1e4 : Math.round(px);
...
export function addOtherAsset({ name, account = "일반", valueKrw, costKrw, isCash = false, kind, trader = "A" }) {
  ...
  list.push({
    ...
    ts: Date.now(), updatedAt: Date.now(), trader, // [C-4] 갱신 시각
  });
  writeOther(list);
  return { ok: true };
}
...
export function updateOtherAsset(id, patch = {}, trader = "A") {
  ...
  next.updatedAt = Date.now(); // [C-4] 갱신 시각
  list[idx] = next;
  writeOther(list);
  return { ok: true };
}
...
export function sellOtherAsset(id, amountKrw, trader = "A") {
  ...
  if (remain <= 0) {
    ...
    markDeleted(OKEY, id); // [C-4] 전량 인출 → 삭제 로그
    return { ok: true, removed: true };
  }
  list[idx] = { ...list[idx], valueKrw: remain, updatedAt: Date.now() };
  writeOther(list);
  return { ok: true, removed: false, remain };
}
export function removeOtherAsset({ id, trader = "A" }) {
  const before = readOther();
  const target = before.find((o) => o.id === id && (o.trader || "A") === trader);
  writeOther(before.filter((o) => !(o.id === id && (o.trader || "A") === trader)));
  if (target) markDeleted(OKEY, id); // [C-4] 삭제 로그
  return { ok: true };
}
