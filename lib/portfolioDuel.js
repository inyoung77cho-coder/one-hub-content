...
//   써야(위 startDuel 주석 참고 — 결정 없이 직접 산 종목도 반영) 하므로, 호출부(컴포넌트)가
//   이미 계산한 실시간 총액을 myValueOverride로 넘기면 그 값을 그대로 스냅샷에 쓴다.
export function recordSnapshot(trader, quotes, myValueOverride) {
  ...
  list.push({ date: t, myValue, aiValue: portfolioValue(p.ai, quotes), prices: quotes || {} });
  all[trader] = list;
