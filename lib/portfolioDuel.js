...
// [S19-4 2026-08-30] 매수 현금 가드 — 잔액 안에서 정수 주수로 축소 체결하고, 한 주도 못 사면 건너뛴다.
//   예전엔 잔액을 보지 않고 cash -= amt 를 했다. AI 쪽은 추천을 전부 수용하므로 현금이 음수로
//   내려갔고(실측 -6,686,793원), 그러면 AI가 무한 신용으로 거래하는 셈이라 총액 비교가 성립하지 않는다.
//   나(me)와 AI 에 같은 규칙을 쓴다 — 한쪽만 제약하면 비교가 반대로 왜곡된다.
function buyWithinCash(cash, positions, d, skipped) {
  const px = Number(d.price) || 0;
  const want = Number(d.qty) || 0;
  const afford = px > 0 ? Math.max(0, Math.min(want, Math.floor(cash / px))) : 0;
  if (afford <= 0) {
    skipped.push({ code: d.code, name: d.name, date: d.date, want, filled: 0, reason: "no_cash" });
    return cash;
  }
  const cur = positions[d.code] || { code: d.code, name: d.name, qty: 0, avgPrice: 0 };
  const totalQty = cur.qty + afford;
  cur.avgPrice = totalQty > 0 ? (cur.avgPrice * cur.qty + px * afford) / totalQty : px;
  cur.qty = totalQty;
  cur.name = cur.name || d.name;
  positions[d.code] = cur;
  if (afford < want) skipped.push({ code: d.code, name: d.name, date: d.date, want, filled: afford, reason: "partial_cash" });
  return cash - afford * px;
}
// 반올림 오차로도 음수가 남지 않게 마지막에 한 번 더 조인다(표시 신뢰).
function clampCash(c) { return c < 0 && c > -1 ? 0 : c; }
...
function applyDecisions(base, decisions, side) {
  ...
  sorted.forEach((d) => {
    ...
    if (d.action === "buy") {
      cash = buyWithinCash(cash, positions, d, skipped);
  ...
  return { cash: clampCash(cash), positions: Object.values(positions).filter((p) => p.qty > 0), skipped };
}
...
// [2026-08-27 버그 수정] 이 날짜 이전 스냅샷은 "AI" 계산 모델 자체가 지금과 다르던
//   시절(구 base+전체수용재계산 모델, 예수금 0원 버그 등)에 찍힌 값이라 지금 값과 이어붙이면
//   그래프가 왜곡된다(사용자 리포트: "계산상의 오류") — 지우지 않고 읽기 시점에만 걸러낸다.
//   8/25 당일 값도 사용자가 오류로 확인·요청해 제외 — 8/26부터 누적 비교.
const SNAPSHOT_VALID_FROM = "2026-08-26";

export function getSnapshots(trader = "A") {
  return (read(SNAPSHOTS_KEY, {})[trader] || [])
    .filter((s) => s.date >= SNAPSHOT_VALID_FROM)
    // [이상치 제외] 시세 조회 실패 등으로 0/음수/결측이 박힌 날은 그래프에서 뺀다(왜곡 방지).
    .filter((s) => Number(s.myValue) > 0 && Number(s.aiValue) > 0)
    .slice()
    .sort((a, b) => (a.date < b.date ? -1 : 1));
}

// [사용자 삭제] 특정 날짜 스냅샷 제거 — 이상하게 찍힌 날(예: 8/27)을 히스토리에서 지운다.
export function removeSnapshot(trader = "A", date) {
  const all = read(SNAPSHOTS_KEY, {});
  const list = all[trader] || [];
  all[trader] = list.filter((s) => s.date !== date);
  write(SNAPSHOTS_KEY, all);
}
...
export function getDecisionAnalysis(trader = "A") {
  ...
  return decisions.map((d) => {
    ...
    ANALYSIS_WINDOWS.forEach((w) => {
      ...
      if (!hit) {
        // [S19-2] '집계 전'만 반복 표시하면 사용자에겐 '눌러도 아무 일 없는 앱'이 된다.
        //   아직 기간이 안 지난 것과, 기간은 지났는데 가격 기록이 없어 채점 자체가 불가능한 것을 나눈다.
        //   후자는 스냅샷 적립(접속 시 1회)이 끊겼다는 뜻이라 사용자가 알아야 한다.
        const due = Date.now() >= targetTs;
        windows[w.key] = { label: w.label, ready: false, reason: due ? "no_price" : "waiting", dueTs: targetTs };
        return;
      }
...
export function computeAiFromLive(myLive, decisions) {
  ...
  const skipped = [];   // [S19-4] AI가 현금이 모자라 못 산 추천 — 카드가 이유를 말할 수 있게 남긴다
  const positions = {};
  ...
  sorted.forEach((d) => {
    if (d.action === "buy") {
      // 나는 안 삼 → AI라면 샀을 것. 단 AI도 예수금 한도 안에서만 산다(S19-4).
      cash = buyWithinCash(cash, positions, d, skipped);
    } else if (d.action === "sell") {
      // 나는 계속 보유 → AI라면 팔았을 것.
      // [버그 수정] cash += amt를 포지션 유무와 무관하게 무조건 더하고 있었다 — 거부한 시점
      // 이후 내가 그 종목을 실제로(독립적으로) 이미 팔아 실시간 포트폴리오에 더 이상 없으면,
      // AI는 없는 걸 판 것처럼 현금만 공짜로 받고 주식 쪽 차감은 없어(if(cur) 가드) 총액이
      // 부풀려졌다(실사용자 리포트: AI 총액이 비정상적으로 큼). 실제로 들고 있는 수량만큼만,
      // 그것도 그 수량이 남아있을 때만 현금화한다.
      const cur = positions[d.code];
      if (cur) {
        const sellQty = Math.min(cur.qty, d.qty);
        cash += sellQty * d.price;
        cur.qty -= sellQty;
        if (cur.qty <= 0) delete positions[d.code];
      }
    }
