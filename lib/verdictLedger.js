...
// AI가 '매수'로 행동하는 기준 점수. 실측(2026-07-19 BEAR) 점수 max=10이라 12면 영영 관망(게임 흥미도 0).
//   → 10으로. 이 미만이면 AI는 '관망'(현금). 국면 강세일수록 점수가 높아져 자연히 AI 매수가 늘어난다.
...
export function computeShowdown(list, windowDays) {
  const rows = list
    .map((e) => ({ e, r: returnAt(e, windowDays), ai: aiBought(e.score) }))
    .filter((x) => x.r != null);
  if (!rows.length) return { ready: false, n: 0 };
  const takes = rows.filter((x) => x.e.decision === "take");
  const aiBuys = rows.filter((x) => x.ai);                 // [규율] AI: 매수판정(score>=12)만 매수
  const avg = (arr) => (arr.length ? Math.round((arr.reduce((a, b) => a + b, 0) / arr.length) * 10) / 10 : 0);
  const aiRet = aiBuys.length ? avg(aiBuys.map((x) => x.r)) : 0; // AI: 확신 종목만(없으면 관망=0)
  ...
  const details = rows
    .map((x) => {
      ...
      return { code: x.e.code, name: x.e.name, decision: x.e.decision, ret: x.r, correct, score: x.e.score ?? null, aiBought: x.ai };
    })
    .sort((a, b) => Math.abs(b.ret) - Math.abs(a.ret));
  return {
    ready: true, n: rows.length, takeN: takes.length, passN: rows.length - takes.length,
    aiBuyN: aiBuys.length, aiPassN: rows.length - aiBuys.length,
    myRet, aiRet, diff, winner, details,
  };
}
