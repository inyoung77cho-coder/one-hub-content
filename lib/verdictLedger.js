...
// [S19-2] 원장 상한. 120은 이미 꽉 찼고(실측 120/120), 그 상태에서 판단을 하나 더 남기면
//   가장 오래된 기록이 조용히 사라진다 — '누적'이 목적인 기능에서 침묵하는 데이터 손실이다.
const LEDGER_MAX = 500;
function write(list) {
  try { localStorage.setItem(KEY, JSON.stringify(list.slice(-LEDGER_MAX))); } catch {}
  // [2026-08-05] 서버 동기화(gameSync.js)가 이 이벤트를 듣고 원장을 서버로 밀어올린다 —
  //   localStorage 단독이던 시절 "매일 1000만원 리셋" 버그(카카오 인앱 등에서 저장 유실)의 근본 수정.
...
export function recordDecision({ code, name, entry, decision, trader = "A", source = "manual", score = null }) {
  ...
  if (idx >= 0) {
    ...
    // [S19-2] 진입가를 뒤늦게 채울 때 snaps 를 시드하지 않아 스냅이 영영 비어 있었다.
    //   matureLedger 는 entry>0 인 항목만 갱신하는데, 시드가 없으면 첫 기준점이 없어 채점이 안 된다.
    if (price && !(Number(list[idx].entry) > 0)) {
      list[idx].entry = price;
      list[idx].entry_backfilled = true;       // 진입가가 '판단 시점'이 아님 — 통계에서 구분 가능하게
      delete list[idx].pending_entry;
      if (!Array.isArray(list[idx].snaps) || list[idx].snaps.length === 0) {
        list[idx].snaps = [{ ts: now, price }];
      }
    }
    ...
    list.push({
      ...
      // [S19-2] 진입가를 못 구한 채로 남긴 판단은 '미확정'으로 표시해 두고 다음 접속 때 백필한다.
      //   실측: '샀어요' 2건이 entry=null·snaps=[] 로 영구 채점 불가 상태였다.
      ...(price ? {} : { pending_entry: true }),
...
export async function matureLedger(trader, fetchPrice) {
  ...
  // [S19-2] ① 진입가가 비어 있는 항목 백필 — 이게 없으면 그 판단은 영원히 채점 대상 밖이다.
  //   진입가가 판단 시점이 아니라 백필 시점이 되므로 entry_backfilled 로 표시해 둔다.
  const needEntry = list.filter((e) => (e.trader || "A") === trader && !(Number(e.entry) > 0));
  await Promise.all(
    needEntry.map(async (e) => {
      try {
        const price = await fetchPrice(e.code);
        if (price > 0) {
          e.entry = price;
          e.entry_backfilled = true;
          delete e.pending_entry;
          e.snaps = [{ ts: now, price }];
        }
      } catch {}
    })
  );
