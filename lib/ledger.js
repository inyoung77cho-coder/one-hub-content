...
export async function getLedger(trader = "A") {
  ...
  //   ★ 다만 '조용히' 진행하면 안 된다. 백엔드가 죽으면 증권사(KIS) 보유가 통째로 빠져
  //     총자산이 경고 없이 수억 줄어든 채 참값처럼 보인다(실측: Upstream 404 시 36.35억 → 34.73억).
  //     틀린 숫자를 자신 있게 보여주는 것이 이 앱이 없애려는 신뢰 훼손 그 자체다 → 반드시 표면화한다.
  ...
  // [N1] 백엔드 원장 부재 → 총자산이 불완전하다는 사실을 숫자 옆에서 말한다.
