...
import { cachedJson } from "./quoteCache"; // [S20-1] getLedger가 한 화면에서 2회 불려도 동일 GET은 1회만
...
// [S20-1] 멱등 GET은 quoteCache 로 30초 캐시·동시요청 중복제거(한 틱에 여러 번 불려도 1회).
const getJson = (url) => cachedJson(url);
...
export async function getLedger(trader = "A", opts = {}) {
  ...
  badAll.forEach((h) => {
    ...
    warnings.push({
      code: "AVG_PRICE_OUT_OF_RANGE", source: "stock", name: h.name,
  ...
  // [S22-1] 평단이 현재가와 10배 이상 어긋난 ETF 보유 — 총자산·손익에서 제외한 사실을 표면화(주식과 동일 UI).
  (live?.excluded || []).forEach((e) => warnings.push({
    code: "AVG_PRICE_OUT_OF_RANGE", source: "etf", name: e.ticker,
    id: e.id, avgPrice: Number(e.avgPrice), avgCcy: e.avgCcy, shares: Number(e.shares),
    message: `${e.ticker} 평단(${Number(e.avgPrice).toLocaleString()}${e.avgCcy === "USD" ? "$" : "원"})이 현재가와 10배 이상 차이나 총자산·손익에서 제외했습니다`,
    excluded_from_totals: true,
  }));
  // [S24-3] 시세 급변(직전 정상가 대비 3배↑/⅓↓) — 평단이 아니라 '시세' 문제. 확인 전 총자산 제외.
  (live?.suspect || []).forEach((s) => warnings.push({
    code: "SUSPECT_PRICE", source: "etf", name: s.ticker, ticker: s.ticker, id: s.id || s.ticker,
    incoming: s.incoming, last: s.last,
    message: `${s.ticker} 시세가 직전 정상가와 3배 이상 차이나 확인 전까지 총자산에서 제외했습니다`,
    excluded_from_totals: true,
  }));
  // [S37-2] 일부 ETF 시세를 아예 못 받아(평단·급변 문제가 아닌 '시세 없음') 합산에서 빠진 경우.
  //   예전엔 q.price==null 이 흔적 없이 통과해 총자산이 조용히 과소표시됐다. 이제 경고로 올린다.
  if (live?.partial && live?.missing?.length) warnings.push({
    code: "ETF_PRICE_MISSING", source: "etf", name: null, count: live.missing.length,
    message: `ETF ${live.missing.length}종목의 시세를 받지 못했습니다 — 이 금액은 실제보다 적습니다`,
    excluded_from_totals: true,
  });
  ...
  // [S23 T-2] 투자용 부동산(순액) — 실거주(대표단지) 분리용. assets.js 가 쓰던 계산을 원장으로 올려
  //   운용자산/실거주를 여기서 한 번만 낸다(두 화면이 같은 필드를 읽게 = N1).
  const invRealtyNetUk = invProps.reduce((s, p) => s + Math.max(0, (Number(p.valueUk) || 0) - (Number(p.deposit) || 0)), 0);
  ...
  // [S23 T-2] 실거주(못 파는 자산) = 부동산 순액 − 투자용 부동산 순액. 운용자산 = 총자산 − 실거주.
  //   실거주가 없으면 residence=0·operating=total(회귀 없음). 화면은 이 필드만 읽는다(다시 빼지 않는다).
  const residence_uk = realestate_uk != null ? round2(Math.max(0, realestate_uk - invRealtyNetUk)) : null;
  const operating_uk = total_uk != null ? round2(Math.max(0, total_uk - (residence_uk || 0))) : null;

  const ledger = {
    ok: total_uk != null,
    as_of: new Date().toISOString(),
    total_uk,
    breakdown: { stock_uk, etf_uk, realestate_uk, cash_uk, stock_won, residence_uk, operating_uk },
