...
export async function fetchLiveEtfKrw(trader = "A") {
  ...
  //   부분 실패면 이 금액은 실제보다 '적다' — ledger 가 경고로 올린다.
  if (any) return { krw: sum, live: true, partial: missing.length > 0, missing, excluded, suspect };
  // 폴백: 실측 시세를 하나도 못 얻은 경우 백엔드 요약값(온전한 평가액) 사용 → partial 아님.
  if (summary?.value_krw != null) return { krw: summary.value_krw, live: false, partial: false, missing, excluded, suspect };
  return { krw: null, live: false, partial: false, missing, excluded, suspect };
}
