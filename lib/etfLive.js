...
export async function fetchLiveEtfKrw(trader = "A") {
  ...
  const missing = []; // [S37-2] 시세를 못 받아 합산에서 빠진 종목(예전엔 흔적 없이 사라져 총자산이 과소표시됐음)
  positions.forEach((p) => {
    const qty = p.qty ?? p.shares ?? p.quantity ?? null;
    if (!(qty > 0)) return;                    // 평가 대상 아님(수량 없음) → 누락 아님
    const q = quotes[p.ticker];
    if (q?.price == null) { missing.push({ ticker: p.ticker }); return; } // [S37-2] 시세 못 받음
    ...
    if (px == null) { missing.push({ ticker: p.ticker }); return; }       // [S37-2] 환율 없어 원화 환산 불가
    sum += qty * px; any = true;
  });
  holdings.forEach((h) => {
    const q = quotes[h.ticker];
    if (q?.price == null) { missing.push({ id: h.id, ticker: h.ticker }); return; } // [S37-2] 시세 못 받음
    ...
    if (curKrw == null) { missing.push({ id: h.id, ticker: h.ticker }); return; }   // [S37-2] 환율 없어 환산 불가
  ...
  // [C-1 2026-09-26] 평가 상태 계약을 명확히 한다. ★partial=false 를 '모든 보유 평가 완료'와 동일시하지 말 것 —
  //   시세 못받음(missing)·평단 오염 제외(excluded)·시세 급변 의심(suspect)은 서로 다른 사유이며,
  //   partial 은 그중 'missing'만 반영한다. '전부 신뢰값으로 합산됐는가'는 fully_evaluated 로 판정한다.
  //   source: 평가 기준 — live(실측 종가) / backend_summary(백엔드 요약 폴백·라이브 아님) / none.
  const fully_evaluated = missing.length === 0 && excluded.length === 0 && suspect.length === 0;
  // [S37-2] 일부만 실패해도 live:true 유지하되 partial:true 를 함께 낸다(부분 실패를 전체 실패로 바꾸지 않음).
  if (any) return { krw: sum, live: true, source: "live", partial: missing.length > 0, fully_evaluated, missing, excluded, suspect };
  // 폴백: 실측 시세를 하나도 못 얻어 백엔드 요약값 사용 → 라이브 아님·완전평가 단정 못함(fully_evaluated=false).
  if (summary?.value_krw != null) return { krw: summary.value_krw, live: false, source: "backend_summary", partial: false, fully_evaluated: false, missing, excluded, suspect };
  return { krw: null, live: false, source: "none", partial: false, fully_evaluated: false, missing, excluded, suspect };
}
