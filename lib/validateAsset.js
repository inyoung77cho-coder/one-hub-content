...
// 주식 직접입력 검증 — 수량 필수. 평단은 선택: 미입력 시 현재가(closePrice)로 자동 계산(종목+수량만).
//   현재가 대비 ±10배 이탈은 단위 오입력으로 간주해 차단.
export function validateStockInput({ shares, price, closePrice, ccy = "KRW" }) {
  ...
  // [현재가 자동] 평단 미입력 → 현재가가 있으면 현재가 기준으로 저장. 현재가도 없으면(해외 등) 평단 요구.
  if (!(p > 0)) {
    if (c > 0) return { ok: true, usedCurrent: true };
    return { ok: false, error: `평단가(${unit})를 입력하거나, 국내 종목은 목록에서 선택해 현재가를 불러오세요.` };
  }
  if (c > 0) {
