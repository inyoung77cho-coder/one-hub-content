...
export function getKrxSession(now) {
  ...
  // [사용자 지시 2026-08-30] 장전 시간외 거래는 08:00 부터 시작한다(기존 08:30 시작 표기를 정정).
  //   08:00~08:30 은 동시호가 이전 구간이라 note 를 나눠 붙인다.
  if (t >= 8 * 60 && t < 8 * 60 + 30) return { key: "pre_close_price", label: "장전 시간외종가(전일종가)", phase: "after" };
