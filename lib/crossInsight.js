...
const RULES = [
  {
    ...
    // ?acct= 값은 lib/etfHoldings.ACCOUNTS 와 정확히 일치해야 필터가 선택된다("연금"은 없는 값 → 무시됨).
    cta: { label: "연금 ETF 보기 →", href: "/pwa/etf?acct=개인연금" },
  },
  {
  ...
  {
    // [S22-10] 환노출 중복 — 주식·ETF의 달러 자산이 운용의 다수면 환율이 자산 전체를 흔든다.
    //   overseasPct(운용자산 중 달러 비중)는 호출측(assets.js)이 보유에서 계산해 넘긴다.
    id: "FX_CONCENTRATION",
    when: (L, m) => Number(m?.overseasPct) >= 60,
    say: (L, m) =>
      `운용자산의 ${r0(m.overseasPct)}%가 달러(해외) 자산입니다 — 환율이 내리면 수익이 나도 원화 평가가 깎일 수 있습니다. ` +
      `국내 자산으로 일부 분산하면 환 변동에 덜 흔들립니다.`,
    cta: { label: "ETF 배분 보기 →", href: "/pwa/etf" },
  },
  {
    id: "CASH_THIN",
    when: (L) => {
