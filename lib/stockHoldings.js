...
export function buyStock({ name, code, shares, avgPrice, ccy = "KRW", broker = "기타", market, account = "일반", buyDate = "", trader = "A" }) {
  ...
  if (idx >= 0) {
    ...
    list.push({ id: `${nm}-${acct}-${ccy}-${trader}-${ts()}`, name: nm, code: cd, shares: qty, avgPrice: roundPx(px), ccy, broker, market: mkt, account: acct, buyDate: buyDate || "", ts: Date.now(), trader });
  }
  write(list);
  return { ok: true };
}
