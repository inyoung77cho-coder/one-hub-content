...
export function buyStock({ name, code, shares, avgPrice, ccy = "KRW", broker = "기타", market, account = "일반", buyDate = "", trader = "A", priceBasis = "manual" }) {
  ...
  const list = read();
  // [S37-4] 병합 키에 증권사(broker)·종목코드(code)를 포함해 '다른 증권사 같은 종목'이 합쳐지지 않게 한다.
  //   예전엔 키가 trader+name+account+ccy 뿐이라 미래에셋 1주 + 한국투자 1주 → 2주(broker=마지막값)로 뭉개졌다.
  //   ★마이그레이션(기존 데이터 보호): 이미 합쳐진 레코드는 쪼개지 않는다(원복 불가). 새 입력부터 새 키로 분리.
  //     한쪽이라도 code 가 비면(레거시·미입력) code 를 무시하고 이름으로 매칭한다(하위 호환).
  const idx = list.findIndex((h) => {
    if ((h.trader || "A") !== trader) return false;
    if ((h.account || "일반") !== acct) return false;
    if ((h.ccy || "KRW") !== ccy) return false;
    if ((h.broker || "") !== (broker || "")) return false;          // 증권사 분리
    const hc = (h.code || ""), inc = (cd || "");
    if (hc && inc) return hc === inc;                                // 양쪽 code 있으면 코드 우선
    return h.name === nm;                                            // 한쪽이라도 비면 이름으로(하위 호환)
  });
  if (idx >= 0) {
    const cur = list[idx];
    ...
    list.push({ id: `${nm}-${acct}-${ccy}-${trader}-${ts()}`, name: nm, code: cd, shares: qty, avgPrice: roundPx(px), ccy, broker, market: mkt, account: acct, buyDate: buyDate || "", ts: Date.now(), updatedAt: Date.now(), trader, priceBasis });
  }
  write(list);
  return { ok: true };
}
...
export function removeStock({ id, trader = "A" }) {
  const before = read();
  const target = before.find((h) => h.id === id && (h.trader || "A") === trader);
  write(before.filter((h) => !(h.id === id && (h.trader || "A") === trader)));
  if (target) markDeleted(KEY, id); // [C-4] 삭제 사실을 로그로 남겨 다른 기기에서 되살아나지 않게
  return { ok: true };
}
