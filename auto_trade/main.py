...
def on_startup():
    ...
    try:
        ...
        print(f"[STARTUP] 보유종목 복구 실패 (장외시간 정상): {e}")
...
def _run_analysis():
    ...
    print(f"Analysis start: {now_kst().strftime('%H:%M')} KST [id={_current_analysis_id}]")
    send("분석 시작...")
    send("ONE-HUB Analysis " + now_kst().strftime("%Y-%m-%d %H:%M"))
    ...
    for stock in top_stocks:
        ...
        if status == "VALID":
            ...
            if code in watchlist_today:
                watchlist_today[code]["classified_at"] = now_kst().strftime("%H:%M")
            else:
                watchlist_today[code] = {
                    "stock": stock, "reason": reason,
                    "classified_at": now_kst().strftime("%H:%M")
                }
        elif status == "ERROR":
...
def us_market_brief():
    try:
        ...
        brief  = f"ONE-HUB Morning Brief {now_kst().strftime('%H:%M')} KST\n\n"
...
def evening_report():
    ...
    with _report_lock:
        ...
        if getattr(evening_report, '_last_date', '') == _today_str:
            print('[DEDUP] evening_report 오늘 이미 실행됨')
            return
        evening_report._last_date = _today_str
        _report_running = True

    # [Fix 3] regime 기본값 선언 → 함수 어디서든 안전하게 사용
    regime = "UNKNOWN"
    try:
        send_info("ONE-HUB Evening Report " + now_kst().strftime("%Y-%m-%d"))
        # [BUG2 Fix] 토큰 만료 시 강제 재발급 후 재시도
        try:
            token   = get_access_token()
            balance = get_balance(token)
        except Exception as _te:
            print(f"[TOKEN RETRY] 토큰 재발급 시도: {_te}")
            import kis_api as _kis
            _kis._token_cache["token"]   = None
            _kis._token_cache["expires"] = 0
            token   = get_access_token()
            balance = get_balance(token)
        cash     = int(balance["output2"][0].get("dnca_tot_amt", 0))
        holdings = balance.get("output1", [])
        ...
        # [v8] final_score / ml_signal 주입

        for _s in top_stocks:

            if not _s.get("final_score"):

                _s["final_score"] = round(_s.get("score", 0), 1)

            if not _s.get("ml_signal") or _s.get("ml_signal") == "?":
        ...
        log_daily(regime, cash + daily_pnl, daily_pnl, trade_count, len(blocked_today))
...
def _handle_single_command(text):
    ...
    if text.startswith("/aimode"):
        ...
        if sub == "on":
            if AI_AUTONOMOUS_MODE:
                send("⚠️ 이미 AI 자율 운영 모드가 활성화 상태입니다.")
                return
